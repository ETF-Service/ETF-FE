...
const MainContent = () => {
  ...
  return (
    <main className="flex-1 bg-gray-950 text-white min-h-screen flex flex-col items-center p-10">
      {/* 타이틀 */}
      <h1 className="text-xl font-semibold mb-4 w-full text-left">ETF 알림 챗봇</h1>
      {/* 챗봇 대화 영역 */}
      <div className="w-full max-w-2xl flex flex-col gap-4 bg-gray-900 rounded-lg p-6 min-h-[300px] mt-4 mb-4">
        {/* 챗봇 메시지 */}
        <div className="flex items-start gap-2">
          <div className="flex-shrink-0 w-8 h-8 bg-yellow-400 text-black rounded-full flex items-center justify-center font-bold">🤖</div>
          <div className="bg-gray-800 rounded-xl px-4 py-2 text-sm max-w-[80%]">
            안녕하세요! ETF 알림 챗봇입니다. 궁금한 점이나 투자 관련 질문을 입력해 주세요.
          </div>
        </div>
        {/* 사용자 메시지 */}
        <div className="flex items-start gap-2 justify-end">
          <div className="bg-blue-700 rounded-xl px-4 py-2 text-sm max-w-[80%] text-white">
            미국 S&P500 ETF에 대해 알려줘
          </div>
          <div className="flex-shrink-0 w-8 h-8 bg-blue-700 text-white rounded-full flex items-center justify-center font-bold">나</div>
        </div>
        {/* 챗봇 답변 */}
        <div className="flex items-start gap-2">
          <div className="flex-shrink-0 w-8 h-8 bg-yellow-400 text-black rounded-full flex items-center justify-center font-bold">🤖</div>
      ...
      <div
        className="fixed bottom-4 w-full max-w-2xl flex items-center bg-gray-900 p-2 rounded border-t border-gray-800 z-50"
